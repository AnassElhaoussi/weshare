  ...
  return (
    ...
    <div className='flex flex-col gap-5 md:w-3/4 h-96 bg-gray-100 dark:bg-gray-800 p-6 rounded-md'>
        ...
        <div className='flex flex-wrap gap-1 justify-start font-bold'>
            {weshareTags.map(tag => (
                <button className='bg-yellow-300 dark:bg-yellow-400 py-1 text-xs w-fit px-3 rounded-2xl hover:scale-105 transition-all' onClick={() => setNewWeshareTag(tag)}>
                    {tag}
                </button> 
            ))}
        </div>
        ...
        <div className='flex justify-between font-bold text-white dark:text-gray-300 '>
            <button className='bg-blue-500 dark:bg-blue-700 py-1 px-3 rounded hover:scale-105 transition-all' onClick={handleEdit}>Edit</button>
            <button className='bg-blue-500 dark:bg-blue-700 py-1 px-3 rounded hover:scale-105 transition-all' onClick={() => setEdit(false)}>Cancel</button>
        </div>
    </div>
